...
const partsToAnthropicContent = (parts: Part[]): string | AnthropicContentBlock[] => {
  ...
  if (hasOnlyText) {
    return items.map((item) => (item.type === 'text' ? item.text : '')).filter(Boolean).join('\n');
  }
  return items;
};
...
export const sendAnthropicMessageNonStream: NonStreamMessageSender = async (
  ...
  try {
    ...
    const response = await fetch(
      ...
      createRequestInit(apiKey, buildAnthropicRequestBody(modelId, history, parts, anthropicConfig, role, false), abortSignal),
    );
    if (!response.ok) {
      throw new Error(await readAnthropicErrorMessage(response));
    }
...
export const sendAnthropicMessageStream: StreamMessageSender = async (
  ...
  try {
    ...
    const response = await fetch(
      buildAnthropicMessagesUrl(anthropicConfig.baseUrl),
      createRequestInit(apiKey, buildAnthropicRequestBody(modelId, history, parts, anthropicConfig, role, true), abortSignal),
    );
    if (!response.ok) {
      throw new Error(await readAnthropicErrorMessage(response));
    }
